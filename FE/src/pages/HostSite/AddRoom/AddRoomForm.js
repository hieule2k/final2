...
const AddRoomForm = () => {
    ...
    useEffect(() => {
        axios.get(`http://103.184.113.181:82/hotel/${location.state.id}/rooms?page=1&limit=10`).then(function (res) {
            // console.log(res);
    ...
    const handleFetchAddRoom = async (values) => {
        try {
            if (location.state.update === 'notUpdate') {
                const res = await axios.post(`http://103.184.113.181:82/hotel/${location.state.id}/room`, JSON.stringify(values));
                console.log(res);
            }
            if (location.state.update === 'update') {
                const res = await axios.put(`http://103.184.113.181:82/room/${location.state.roomId}`, JSON.stringify(values));
                console.log(res);
            }

            rooms.length > 0 ? navigate('/AddRoom', { state: { id: location.state.id } }) : navigate('/HostProperties');
        } catch (error) {
            if (error.response) {
                    ...
                    <form onSubmit={formik.handleSubmit}>
                        <div className="flex flex-col gap-8 p-24">
                            ...
                            <label className="flex gap-4 ">
                                ...
                                <Field className="text-lg font-semibold" name="type" as="select">
                            ...
                            <button className="w-32 h-12 px-4 py-2 rounded bg-[#98FB98] self-end">Submit</button>
                        </div>
                    </form>
                )}
            </Formik>
