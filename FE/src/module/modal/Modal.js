...
const Modal = ({ userId, hotel, handleModalVisible, userName }) => {
    ...
    const handleFetchComment = async (values) => {
        try {
            const res = await axios.post(`http://103.184.113.181:81/hotel/add_comment`, JSON.stringify(values));
            console.log(res);
        } catch (error) {
            if (error.response) {
                    ...
                    <div className={cx('modal')}>
                        <div className={cx('modal-content')}>
                            <div className={cx('top-wrapper')}>
                                <h2>Review</h2>
                                <CloseIcon customclass={cx('close-icon')} onClick={handleModalVisible}></CloseIcon>
                            </div>
                                    ...
                                    {new Array(5).fill(0).map((star, i) => {
                                        ...
                                        return (
                                            <button
                                                type="button"
                                                key={i}
                                                className={i <= (hover || rating) ? cx('on') : cx('off')}
                                                onClick={() => handleSetRating(i, formik.setFieldValue)}
                                                onMouseEnter={() => setHover(i)}
                                                onMouseLeave={() => setHover(rating)}
                                            >
                                                <StarIcons customclass={cx('star-icon')} key={i}></StarIcons>
                                            </button>
                                        );
                                    })}
                                </div>
                            ...
                            <div>
                                ...
                                <TextAreaFormik name="text" placeholder="Enter your review" id="intro" className={cx('text-area')}></TextAreaFormik>
                            </div>
                            <div className={cx('button-wrapper')}>
                                <Button type="button" bgGray small black onClick={handleModalVisible}>
                                    Cancel
                                </Button>
